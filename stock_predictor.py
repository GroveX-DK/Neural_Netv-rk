...
Træner på data indtil 2025, forudsiger fra start 2025 til start 2026 (facit).
"""
...
BATCH_SIZE = 8            # Mindre = langsommere træning, ofte bedre (fx 16)
EPOCHS = 2               # Flere epoker = længere træning (fx 300-500)
TEST_START_YEAR = 2025   # Forudsigelse fra start 2025 til start 2026
LEARNING_RATE = 0.0001     # Lavere = langsommere læring (fx 0.0001-0.001)
VALIDATION_SPLIT = 0.05    # Sidste 15% af træningsdata til validering
USE_EARLY_STOPPING = True  # False = kør alle EPOCHS uden at stoppe tidligt
EARLY_STOPPING_PATIENCE = 50  # Kun brugt hvis USE_EARLY_STOPPING=True
EARLY_STOPPING_MIN_DELTA = 1e-4  # = 0.000001. Kun tæl som "forbedring" hvis val_loss falder mere end dette
# ====================================================


def load_and_prepare_data(csv_path: str):
    """Læs CSV. Træner på data før 2025, forudsiger fra start 2025 til start 2026."""
...
def train_model(model, train_loader, val_loader, device):
    """Træn modellen med early stopping. Returnerer (model, actual_epochs, stopped_early, best_val_loss).
    stopped_early=True hvis early stopping triggede. Re-raiser KeyboardInterrupt ved Ctrl+C."""
    ...
    try:
        ...
        if best_state is not None:
            model.load_state_dict(best_state)
        raise  # Re-raise så main() kan stoppe genstarts-loopet
...
def save_results_md(
    ...
    # Billedet vises i Markdown-preview (Ctrl+Shift+V i VS Code/Cursor) når .md og .png er i samme mappe
...
def plot_results(actuals, predictions, test_dates, save_path: str):
    ...
    plt.title("Goldman Sachs: Faktisk vs. forudsigelse (2025 til 2026)")
    plt.legend()
...
def main():
    ...
    print(f"Træningsdata: {len(train_df)} rækker (indtil 2025)")
    print(f"Testdata (facit): {len(test_df)} rækker (2025-01-01 til 2026-01-01)")
    ...
    # 6. Model og træning (genstarter ved early stopping, stopper kun ved Ctrl+C)
    model = None
    actual_epochs = 0
    run_number = 0
    model_path = Path(__file__).parent / "stock_model.pt"
    # Indlæs tidligere bedste test MSE (facit), så vi kun gemmer når vi kommer tættere på det rigtige svar
    ...
    def save_model_if_better(current_model, test_mse, scaler, feature_cols, train_loss=None):
        """Gem model kun hvis den kommer tættere på facit (lavere test MSE)."""
        ...
        if test_mse < best_overall_test_mse:
            ...
            print(f"  → Model GEMT (test MSE: {test_mse:.6f} – tættere på facit)")
        else:
            print(f"  → Model IKKE gemt (test MSE: {test_mse:.6f} – ikke bedre end tidligere)")
    ...
    try:
        while True:
            ...
            model, actual_epochs, stopped_early, _best_val_loss, best_train_loss = train_model(model, train_loader, val_loader, device)
            # Evaluer på facit (test) – kun gem hvis vi kommer tættere på det rigtige svar
            _, _, metrics = evaluate_model(model, test_loader, device)
            test_mse = metrics["MSE"]
            save_model_if_better(model, test_mse, scaler, feature_cols, best_train_loss)
            if stopped_early:
                print("Early stopping – genstarter træning... (Ctrl+C for at stoppe)")
            else:
                print("Alle epoker færdige – genstarter træning... (Ctrl+C for at stoppe)")
    except KeyboardInterrupt:
        print("\n\nStoppet af bruger (Ctrl+C).")
        if model is not None:
            # Evaluer den afbrudte runs model på facit for at se om den er bedre
            _, _, metrics = evaluate_model(model, test_loader, device)
            test_mse = metrics["MSE"]
            save_model_if_better(model, test_mse, scaler, feature_cols, train_loss=None)
        else:
            print("Ingen model at gemme.")
            return

    # 8. Indlæs bedste model og evaluer på testdata (facit)
    print("\nIndlæser bedste model og evaluerer på testdata (facit)...")
    ...
    print("\n--- Resultater (facit = data 2025 til 2026) ---")
    ...
    # 10. Plot og resultat-rapport (organiseret i mapper pr. dag)
    ...
    save_results_md(actuals_orig, predictions_orig, test_dates, metrics, timestamp, results_path, plot_path.name, actual_epochs, saved_train_loss, saved_test_mse)

    # Modellen er allerede gemt af save_model_if_better (kun når den er bedre) – ingen overskrivning her
